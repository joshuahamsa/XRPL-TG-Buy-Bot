...
# User-configurable option for filtering by purchase amount
MIN_PURCHASE_AMOUNT = float(input("Enter the minimum token purchase amount to post (e.g., 100): "))
        ...
        while True:
            ...
            for txn in tx:
                if txn['TransactionType'] == 'OfferCreate' and txn['TakerGets']['currency'] == TOKEN_CURRENCY:
                    ...
                    buyer = txn['Account']
                    token_amount = float(txn['TakerGets']['value'])
                    price = txn['TakerPays']['value']
                    
                    # Filter by minimum purchase amount
                    if token_amount >= MIN_PURCHASE_AMOUNT:
